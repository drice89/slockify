...
const ConversationHeader = ({conversation, users, currentUserId, toggleSidebar}) => {
  let headerValue = conversation.convoType === "channel" ? conversation.name : conversation.memberIds
  if (conversation.convoType !== "channel") {
    let members = compact(headerValue.map((id) => {
      return id !== currentUserId ? (users[id].displayName || users[id].fullName) : ""
    }))
     headerValue = members.length > 1 ? members.join(", ").replace(/\s,\s/ig, "").trim() : members[0]
  }
