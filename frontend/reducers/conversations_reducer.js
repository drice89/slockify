...
const conversationsReducer = (state = {}, action) => {
  ...
  switch (action.type) {
    ...
    case RECEIVE_MESSAGE:
      nextState[action.message.recipientId].lastMessage = action.message;
      return nextState;
    case REMOVE_MESSAGE:
      if (nextState[action.message.recipientId].lastMessage.id === action.message.id) {
        nextState[action.message.recipientId].lastMessage = undefined
      }
      return nextState;
    //in case I want to revert state back to hold all messages
    // case RECEIVE_MESSAGE:
